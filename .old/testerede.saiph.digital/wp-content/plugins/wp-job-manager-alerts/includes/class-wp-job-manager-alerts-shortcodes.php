				...
				switch ( $this->action ) {
					...
					case 'edit':
						if ( isset( $_POST['submit-job-alert'] ) ) {
							...
							wp_safe_redirect( add_query_arg( 'updated', 'true', remove_query_arg( array( 'action', 'alert_id' ) ) ) );
							exit;
						}
						break;
					case 'toggle_status':
						...
						// Message
						$this->alert_message = '<div class="job-manager-message">' . sprintf( __( '%1$s has been %2$s', 'wp-job-manager-alerts' ), $alert->post_title, $alert->post_status == 'draft' ? __( 'Enabled', 'wp-job-manager-alerts' ) : __( 'Disabled', 'wp-job-manager-alerts' ) ) . '</div>';

						wp_safe_redirect( add_query_arg( 'updated', 'true', remove_query_arg( array( 'action', 'alert_id', '_wpnonce' ) ) ) );
