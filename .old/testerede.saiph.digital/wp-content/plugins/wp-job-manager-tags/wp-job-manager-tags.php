...
class WP_Job_Manager_Job_Tags {
	...
	public function __construct() {
		define( 'JOB_MANAGER_TAGS_VERSION', '1.4.5' );
	...
	public function version_check() {
		if ( ! class_exists( 'WP_Job_Manager' ) || ! defined( 'JOB_MANAGER_VERSION' ) ) {
			...
			$this->display_error( sprintf( __( '<em>WP Job Manager - Tags</em> requires WP Job Manager %1$s (you are using %2$s).', 'wp-job-manager-tags' ), self::JOB_MANAGER_CORE_MIN_VERSION, JOB_MANAGER_VERSION ) );
		}
	}
	...
	public function register_taxonomy() {
		...
		register_taxonomy(
			...
			array(
				...
				'show_in_rest'          => true,
				'rewrite'               => array(
					'slug'       => _x( 'job-tag', 'permalink', 'wp-job-manager-tags' ),
					'with_front' => false,
				),
			)
		);
	...
	private function is_shortcode_page() {
		...
		foreach ( $shortcodes as $shortcode ) {
			if ( null !== $content && has_shortcode( $content, $shortcode ) ) {
				return true;
			}
		}
